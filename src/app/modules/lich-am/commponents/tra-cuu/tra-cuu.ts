...
type NgocHapItem = { k: string; type: 'good' | 'bad'; v: string };
type GioXuatHanhItem = { chi: string; range: string; note: string };
type NhiThapBatTuItem = {
  key: string;
  data: { sao: string; loai: string; moTa: string; nen: string; ky: string; ngoaiLe?: string };
};
type TrucItem = { key: string; data: { tot: string; xau: string } };
type XungHopValue = { lucHop?: string; tamHop?: string[]; xung?: string; hinh?: string; hai?: string; pha?: string; tuyet?: string };
type RenderResult = {
  solarDate: string;
  lunarDate: string;
  lunarLeap: boolean;
...
export class TraCuu implements OnInit {
  ...
  private getXungHopValue(chi: string) {
    return {
